...
def main():
    ...
    def update(frame):
        for message in consumer:
            ...
            ax.clear()
            ax.plot(car_x_data, car_y_data, 'bo-', label='Car')          # Blue
            ax.plot(gps1_x_data, gps1_y_data, 'rx--', label='GPS1')      # Red
            ax.plot(gps2_x_data, gps2_y_data, 'g^--', label='GPS2')      # Green

            ax.set_xlim(0, 900)  # Adjust if needed
            ax.set_ylim(0, 600)
